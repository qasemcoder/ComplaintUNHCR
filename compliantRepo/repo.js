...
function Send_To_Database(name,phone,casenumber,dis,block,home,reg,message,ind,color,callback) {
    let sql = `INSERT INTO ${DB_NAME}.save (name, phone, casenumber,dis, block, home, reg, message, ind ,color) VALUES ('${name}', '${phone}', '${casenumber}', '${dis}', '${block}', '${home}', '${reg}', '${message}', '${ind}','${color}');`
...
function put_new_color(color , callback) {
    let sql = `UPDATE ${DB_NAME}.save SET color = '1' WHERE (id = '${color}');`
...
function get_color_number(callback) {
    let sql = `select * from ${DB_NAME}.save;`
...
module.exports= {get_color_number,put_new_color,check_email,get_ALL_search_from_Database,Delete_N_Database,Send_Login_To_Database,Send_To_Database,get_name_ind_from_Database,get_All_from_Database,get_search_from_Database, get_All_News_from_Database, Send_news_To_Database}